...
namespace IAUBackEnd.Admin.Controllers
{
    public class Service_TypeController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetActiveService_TypeCharList()
        {
            try
            {
                var data = p.Service_Type.Where(q => q.IS_Action.Value && !q.Deleted).Select(q => q.Service_Type_Name_EN).ToList().Select(q => q.ElementAt(0));
        ...
        {
            return Ok(new ResponseClass() { success = true, result = p.Service_Type.Where(q => q.IS_Action.Value && !q.Deleted) });
        }
        ...
        {
            var data = p.Service_Type.FirstOrDefault(q => q.Service_Type_ID == service_Type.Service_Type_ID && !q.Deleted);
        ...
        public async Task<IHttpActionResult> Create(ServiceTypeDTO service_Type)
        {
            ...
            try
            {
                ...
                p.Service_Type.Add(new Service_Type() { IS_Action = true, Deleted = false, Service_Type_Name_AR = service_Type.Service_Type_Name_AR, Service_Type_Name_EN = service_Type.Service_Type_Name_EN, Image_Path = FilePath.Replace('\\', '/') });
