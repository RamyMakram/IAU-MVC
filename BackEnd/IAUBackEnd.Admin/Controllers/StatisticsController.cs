...
namespace IAUBackEnd.Admin.Controllers
{
    public class StatisticsController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Get()
        {
            ...
            var Locations = p.Units_Location.Where(q => !q.Deleted).Select(q => new { AR = q.Units_Location_Name_AR, EN = q.Units_Location_Name_EN, Count = (int?)q.Units.Sum(w => w.Request_Data.Count) ?? 0 }).ToList();
