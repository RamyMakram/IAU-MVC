...
namespace AdminPanel.Controllers
{
    ...
    {
        // GET: Units
        public ActionResult Home(string uname)
        {
            var Data = APIHandeling.getData("Units/GetUnits");
            var resJson = Data.Content.ReadAsStringAsync();
            var res = JsonConvert.DeserializeObject<ResponseClass>(resJson.Result);
            if (res.success)
                return View(JsonConvert.DeserializeObject<ICollection<UnitsDTO>>(res.result.ToString()).Where(q => uname == null ? true : q.Units_Name_EN.Equals(uname)).ToList());
            else
                return RedirectToAction("NotFound", "Error");
        }
        public ActionResult AllDel()
        {
            var Data = APIHandeling.getData("Units/GetDeleted");
            var resJson = Data.Content.ReadAsStringAsync();
            var res = JsonConvert.DeserializeObject<ResponseClass>(resJson.Result);
            if (res.success)
                return PartialView("DeletedModelsView", JsonConvert.DeserializeObject<ICollection<UnitsDTO>>(res.result.ToString()));
            else
        ...
        {
            var Data = APIHandeling.Post("Units/_Restore?id=" + id, new { });
            var resJson = Data.Content.ReadAsStringAsync();
            var res = JsonConvert.DeserializeObject<ResponseClass>(resJson.Result);
            return Json(res);
        }
