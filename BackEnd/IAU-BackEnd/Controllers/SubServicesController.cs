...
namespace IAU_BackEnd.Controllers
{
    public class SubServicesController : ApiController
    {
        ...
        {
            var res = p.Sub_Services.Include(q => q.Required_Documents).Where(q =>
                !q.Deleted &&
                q.IS_Action.Value &&
                q.Main_Services_ID == MainService &&
                !q.Main_Services.Deleted
            ).Select(q => new { ID = q.Sub_Services_ID, Name_AR = q.Sub_Services_Name_AR, Name_EN = q.Sub_Services_Name_EN, Docs = q.Required_Documents.Where(s => !s.Deleted).Select(s => new { s.Name_AR, s.Name_EN, s.ID }) }).ToList();
