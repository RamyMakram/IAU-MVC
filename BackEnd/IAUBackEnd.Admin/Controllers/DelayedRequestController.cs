...
namespace IAUBackEnd.Admin.Controllers
{
	public class DelayedRequestController : ApiController
	{
		...
		public async Task<IHttpActionResult> GetActive()
		{
			...
			var data = p.DelayedTransaction.Where(q => EntityFunctions.DiffMonths(date, q.AddedDate) == 0).OrderByDescending(q => q.AddedDate).Select(q => new { ReqID = q.RequestID, RequestStatusAR = q.Request_State.StateName_AR, RequestStatusEN = q.Request_State.StateName_EN, q.Readed, q.ID });
