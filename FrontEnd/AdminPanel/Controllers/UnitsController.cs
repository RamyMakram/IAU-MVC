...
namespace AdminPanel.Controllers
{
    public class UnitsController : BaseController
    {
        ...
        [HttpPost]
        public JsonResult CheckCode(string code, int? uID, int ty, int levelID)
        {
            var Data = APIHandeling.Post($"Units/_CheckEnteredCode?code={code}&unitid={uID}&Type={ty}&level={levelID}", new { });
            var resJson = Data.Content.ReadAsStringAsync();
            var res = JsonConvert.DeserializeObject<ResponseClass>(resJson.Result);
            return Json(res);
        }
