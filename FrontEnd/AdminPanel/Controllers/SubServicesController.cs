...
namespace AdminPanel.Controllers
{
    ...
    {
        public ActionResult Home(string sname)
        {
            ...
            if (res.success)
                return View(JsonConvert.DeserializeObject<ICollection<SubServicesDTO>>(res.result.ToString()).Where(q => sname == null ? true : q.Sub_Services_Name_EN.Equals(sname)).ToList());
            else
                return RedirectToAction("NotFound", "Error");
        }
        public ActionResult AllDel()
        {
            var Data = APIHandeling.getData("Sub_Services/GetDeleted");
            var resJson = Data.Content.ReadAsStringAsync();
            var res = JsonConvert.DeserializeObject<ResponseClass>(resJson.Result);
            if (res.success)
                return PartialView("DeletedModelsView", JsonConvert.DeserializeObject<ICollection<SubServicesDTO>>(res.result.ToString()));
            else
        ...
        {
            var Data = APIHandeling.Post("Sub_Services/_Restore?id=" + id, new { });
            var resJson = Data.Content.ReadAsStringAsync();
            var res = JsonConvert.DeserializeObject<ResponseClass>(resJson.Result);
            return Json(res);
        }
        ...
        {
            var Data = APIHandeling.Post("Sub_Services/_Delete?id=" + id, new { });
            var resJson = Data.Content.ReadAsStringAsync();
            var res = JsonConvert.DeserializeObject<ResponseClass>(resJson.Result);
            return Json(res);
        }
