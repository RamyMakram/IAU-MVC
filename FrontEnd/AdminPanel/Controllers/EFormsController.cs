...
namespace AdminPanel.Controllers
{
	public class EFormsController : BaseController
	{
		...
		{
			var Data = APIHandeling.getData("E_Forms/GetDeleted");
			var resJson = Data.Content.ReadAsStringAsync();
			var res = JsonConvert.DeserializeObject<ResponseClass>(resJson.Result);
			if (res.success)
				return PartialView("DeletedModelsView", JsonConvert.DeserializeObject<ICollection<E_FormsDTO>>(res.result.ToString()));
			else
		...
		{
			var Data = APIHandeling.Post("E_Forms/_Restore?id=" + id, new { });
			var resJson = Data.Content.ReadAsStringAsync();
			var res = JsonConvert.DeserializeObject<ResponseClass>(resJson.Result);
			return Json(res);
		}
		...
		public void LoadCreateOrEdit()
		{
			if (Request.QueryString["SubService"] == null)
			{
				...
				var Data1 = APIHandeling.getData("Sub_Services/GetActive");
				var resJson1 = Data1.Content.ReadAsStringAsync();
				var res = JsonConvert.DeserializeObject<ResponseClass>(resJson1.Result);
			...
			var Data = APIHandeling.getData("Units/GetActive");
			var resJson = Data.Content.ReadAsStringAsync();
			var res2 = JsonConvert.DeserializeObject<ResponseClass>(resJson.Result);
			var Units = JsonConvert.DeserializeObject<ICollection<UnitsDTO>>(res2.result.ToString());
			ViewBag.UnitID = new SelectList(Units, "Units_ID", Request.Cookies["lang"]?.Value == "ar" ? "Units_Name_AR" : "Units_Name_EN");
		}
